...
# Read Files
print("Reading 1....")
PATH = "D:\\University of Essex\\data science\\Assignment_final\\CE888-Nurses2\\output"

df = pd.read_parquet(os.path.join(PATH, 'merged_data_parquet.parquet'))
df['datetime'] = pd.to_datetime(df['datetime'].apply(lambda x: x * (10 ** 9)))

print("Reading 2 ...")
survey_path = 'D:\\University of Essex\\data science\\Assignment_final\\CE888-Nurses2\\data\\SurveyResults.xlsx'
...
if __name__ == '__main__':
    ...
    for id in ids:
        results.append(parallel(id))

    new_df = pd.concat(results, ignore_index=True)

    print('Saving ...')
    new_df.to_csv(os.path.join(PATH, 'merged_data_labeled.csv'), index=False)
    # new_df.to_parquet(os.path.join(PATH, 'merged_data_labeled_parquet.parquet'), index=False)
    print('Done')